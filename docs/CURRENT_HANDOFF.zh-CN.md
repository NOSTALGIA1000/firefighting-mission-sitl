...
场地、1.2m OFFBOARD 飞行、固定地图 A*、双目视觉绕桩、双通道投放均已实现。2026-08-31 复审修掉了两个会直接判负的避障缺陷（合法抽签下航线规划失败、偏航速率使 3 分钟赛程在算术上无法完成），新增不依赖 VM 的闭环避障回归测试（24 种抽签布局零碰撞），并在 VM 实跑中定位并修复了两个环境级根因（Gazebo 物理输出 NaN、偏航控制器增益被调到默认值 1/9）。此前基于 EKF 参数的诊断结论已失效。当前不是验收完成版：Gazebo 端到端任务用时仍未测得。

## 2026-08-31 避障复审与修复

本轮以竞赛规则（附件 4-1）为基准全面复审避障链路，按 TDD 修复。全部本地测试 `222/222` 通过。
...
- 最近圆柱圆心 `0.611m`（碰撞阈值 `0.30m`），最近挡板面 `0.457m`（阈值 `0.20m`）。
- 最差任务用时 `95.9s`，3 分钟赛程剩约 `84s` 给悬停、识别、投放、降落。
- 双目测距偏差 `±0.10m` 内仍全过。
...
1. `catkin_make` + 全量测试，确认 222 项在 VM 同样通过。
...
- 非正常降落后规划器可能永久卡死：落点若落在固定挡板 `0.45m` 膨胀区内，`plan_route` 抛 `start_blocked`，日志持续刷 `route rejected: start_blocked`。起点需要与目标点同类的松弛处理，**尚未修复**。
...
`altitude_out_of_band`：真值高度 `1.25m`、容差带 `1.25±0.15`，规划器仍报该原因并保持 `HOLD_UNSAFE`，任务链不推进；`mission_manager` 在仿真 `167` 秒后自行判定 `mission complete` 退出并拖垮 launch。**Gazebo 端到端任务用时仍未测得。**
