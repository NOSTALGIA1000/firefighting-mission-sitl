...
场地、1.2m OFFBOARD 飞行、固定地图 A*、双目视觉绕桩、双通道投放均已实现。2026-08-31 复审修掉两个会直接判负的避障缺陷，新增不依赖 VM 的闭环避障回归测试（24 种抽签布局零碰撞），并在 VM 实跑中定位修复三个环境级根因：Gazebo 物理输出 NaN、偏航控制器增益被调到默认值 1/9、`EKF2_EVA_NOISE` 从未设置导致转向时航向估计跟不上进而位置发散。修复后已实测**自主避障到危险品任务区 40.3s、到人员救助任务区 78.2s**，最近圆柱 0.588m（碰撞阈值 0.30m）。此前基于 EKF 参数的诊断结论已失效。当前不是验收完成版：返航段仍会发散，完整比赛链未跑通。
...
- **返航段仍会发散**。修复后健康窗口约 `132s`，两段任务航线合计约 `118s` 落在窗口内，返航段落在窗口外。`EKF2_EVA_NOISE` 已到参数下限 `0.01`，再往下需要换思路（候选：`EKF2_EV_DELAY` 按实测管线延迟标定；或降低航线转向速率换取裕度）。
- **单次运行结论不可靠**。同一配置连跑数轮结果差异很大（危险品区 `40.3s / 53.1s / 58.7s`，救助区 时好时坏）。需要「同配置连跑 N 轮统计通过率」的对照，而不是单次判定。
